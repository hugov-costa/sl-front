...
export function GrossWeightInput({
  disabled,
  form,
  initialValue,
}: GrossWeightInputProps) {
  const { value, onChange, setRaw, numericValue } = useNumberMask(10, 3);
  const { field } = useController({
    control: form.control,
    name: "gross_weight",
  });
  const hasError = !!form.formState.errors.gross_weight;
  ...
  return (
    <Field data-invalid={hasError}>
      <FieldLabel htmlFor="gross-weight">Peso Bruto (kg)</FieldLabel>
      <Input
        ...
        value={value}
      />
      <FieldErrorZod errors={[form.formState.errors.gross_weight]} />
    </Field>
  );
}
